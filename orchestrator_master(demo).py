...
DATA_DIR    = "data"
AGENTS_DIR  = "agents"
TRUST_FLOOR = 0.05   # No agent trust ever goes below this
# ───────────────────────────────────────────────────────────────
...
async def evaluate_dataset(dataset_name: str, attack_csv: str, normal_csv: str, rows_per_class: int = 5000):
    ...
    with open(os.path.join(MODELS_DIR, 'ae_model_params.pkl'), 'rb') as f: loaded_params = pickle.load(f)
    with open(os.path.join(MODELS_DIR, 'ae_scaler.pkl'), 'rb') as f: loaded_scaler_ae = pickle.load(f)
    loaded_autoencoder = Autoencoder(loaded_params['input_dim'], loaded_params['encoding_dim']).to(device)
    loaded_autoencoder.load_state_dict(torch.load(os.path.join(MODELS_DIR, 'autoencoder_final.pt'), map_location=device))
    loaded_autoencoder.eval()

    scaler_agents = joblib.load(os.path.join(MODELS_DIR, "scaler_agents.pkl"))
    ...
    # ── Rolling accuracy counters (FIX #1) ─────────────────────
    correct_preds = 0
    total_preds   = 0
    # ───────────────────────────────────────────────────────────

    params_mqtt  = StdioServerParameters(command="python3", args=[os.path.join(AGENTS_DIR, "mqtt_agent.py")])
    params_vol   = StdioServerParameters(command="python3", args=[os.path.join(AGENTS_DIR, "volumetric_agent.py")])
    params_recon = StdioServerParameters(command="python3", args=[os.path.join(AGENTS_DIR, "reconnaissance_agent.py")])
    ...
    async with stdio_client(params_mqtt) as (r1, w1), stdio_client(params_vol) as (r2, w2), stdio_client(params_recon) as (r3, w3):
        async with ClientSession(r1, w1) as sess_mqtt, ClientSession(r2, w2) as sess_vol, ClientSession(r3, w3) as sess_recon:
            ...
            for idx, raw_features in enumerate(raw_array):
                ...
                # ── Trust floor clamp (FIX #2) ──────────────────
                curr_tr1 = max(curr_tr1, TRUST_FLOOR)
                curr_tr2 = max(curr_tr2, TRUST_FLOOR)
                curr_tr3 = max(curr_tr3, TRUST_FLOOR)
                # ────────────────────────────────────────────────
                ...
                if mse <= loaded_params['threshold_95']:
                    # Autoencoder flagged as normal — skip agent calls
                    pred = 0
                    y_probs_fused.append(0.0); y_preds_fused.append(0)
                    total_preds += 1
                    correct_preds += (1 if pred == true_label else 0)
                    continue
                ...
                p1 = float(res_m.content[0].text)
                p2 = float(res_v.content[0].text)
                p3 = float(res_r.content[0].text)

                fused_prob = ((curr_tr1 * p1) + (curr_tr2 * p2) + (curr_tr3 * p3)) / (curr_tr1 + curr_tr2 + curr_tr3 + 1e-9)
                pred = 1 if fused_prob >= 0.5 else 0
                y_probs_fused.append(fused_prob); y_preds_fused.append(pred)

                # ── Rolling accuracy update (FIX #1) ────────────
                total_preds += 1
                correct_preds += (1 if pred == true_label else 0)
                rolling_acc = round((correct_preds / total_preds) * 100, 2)
                # ────────────────────────────────────────────────
                ...
                if idx % 50 == 0:
                    retrain_size = 0
                    if os.path.exists("master_retrain_queue.csv"):
                        with open("master_retrain_queue.csv") as f:
                            retrain_size = sum(1 for _ in f)

                    live_status = {
                        "dataset_name":      dataset_name,
                        "packets_processed": idx,
                        "total_packets":     total_rows,
                        "accuracy":          rolling_acc,          # FIX #1: real live accuracy
                        "trust_recon":       round(curr_tr3, 6),   # FIX #2: clamped trust
                        "trust_mqtt":        round(curr_tr1, 6),
                        "trust_vol":         round(curr_tr2, 6),
                        "latest_fused_prob": round(float(fused_prob) * 100, 2),
                        "p_recon":           round(p3 * 100, 2),
                        "p_mqtt":            round(p1 * 100, 2),
                        "p_vol":             round(p2 * 100, 2),
                        "is_attack":         bool(pred == 1),
                        "actual_label":      bool(true_label == 1),
                        "retrain_queue_size": retrain_size,
                        "status":            "running"
                    }
                    _safe_write_json("dashboard_feed.json", live_status)

    acc = accuracy_score(y_true, y_preds_fused)
    exec_time = time.time() - start_time
    print(f"[RESULTS] {dataset_name} Complete | Accuracy: {acc*100:.2f}% | Exec Time: {exec_time:.2f}s")

    # Write final status so dashboard shows completion banner
    retrain_size = 0
    if os.path.exists("master_retrain_queue.csv"):
        with open("master_retrain_queue.csv") as f:
            retrain_size = sum(1 for _ in f)

    _safe_write_json("dashboard_feed.json", {
        "dataset_name":      dataset_name,
        "packets_processed": total_rows,
        "total_packets":     total_rows,
        "accuracy":          round(acc * 100, 2),
        "trust_recon":       round(curr_tr3, 6),
        "trust_mqtt":        round(curr_tr1, 6),
        "trust_vol":         round(curr_tr2, 6),
        "latest_fused_prob": 0.0,
        "p_recon":           0.0,
        "p_mqtt":            0.0,
        "p_vol":             0.0,
        "is_attack":         False,
...
if __name__ == "__main__":
    ...
    datasets = {
        "CIC_IoMT_2024": {"attack": os.path.join(DATA_DIR, "CICIoMT2024_labelled_CICFlow.csv"),  "normal": os.path.join(DATA_DIR, "Benign_test.pcap.csv")},
        "BOT_IOT":       {"attack": os.path.join(DATA_DIR, "BOT_IOT_DDOS.csv"),                   "normal": os.path.join(DATA_DIR, "botiot_ben.csv")},
        "TON_IOT":       {"attack": os.path.join(DATA_DIR, "TON_IOT_MAL.csv"),                    "normal": os.path.join(DATA_DIR, "TON_IOT_Benig.csv")},
        "CIC_DDoS":      {"attack": os.path.join(DATA_DIR, "cicddos_mal.csv"),                    "normal": os.path.join(DATA_DIR, "cicddos_benign.csv")},
        "CIC_IDS":       {"attack": os.path.join(DATA_DIR, "CICIDS_Malicious.csv"),               "normal": os.path.join(DATA_DIR, "CICIDS_Benign.csv")},
    }

    results_summary = []
    for ds_name, paths in datasets.items():
        res = asyncio.run(evaluate_dataset(ds_name, paths["attack"], paths["normal"], 2500))
        if res: results_summary.append(res)

    if results_summary:
        df_res = pd.DataFrame(results_summary)
        df_res.to_csv("Master_Evaluation_Table.csv", index=False)
        print("\n[DONE] All datasets complete. Results saved to Master_Evaluation_Table.csv")
